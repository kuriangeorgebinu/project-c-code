package com.coderofjava8888.main;
...
import com.coderofjava8888.contracts.*;
import com.coderofjava8888.helpers.Portfolio_Assessment;
import com.coderofjava8888.helpers.Position_Processor;
import com.coderofjava8888.strategy.NewStrategy;
...
//  4000  live account  - IB Gateway
        ...
        LiveContract liveContract_05 = new LiveContract(Contracts_202303_MAR.CAD_FXFutureContract(), "CAD", 310735300, 4110.00, 3288.00, 37.00, 32.00, 3003, 19005);
        LiveContract liveContract_06 = new LiveContract(Contracts_202303_MAR.MXP_FXFutureContract(), "MXP", 310735300, 4110.00, 3288.00, 37.00, 32.00, 3004, 19005);
        ...
        portfolioVault.put("CAD", liveContract_05);
        portfolioVault.put("MXP", liveContract_06);
        ...
        activeContracts.add(portfolioVault.put("CAD", liveContract_05));
        activeContracts.add(portfolioVault.put("MXP", liveContract_06));
...
//////////////// /*We always want to start with this because whenever coming from a weekend or an extended period of no activity, the position and portfolio callback method will return empty */
            ...
            if (wrapper.getHashMap_UpdatePortfolio().isEmpty() && wrapper.getPositions().isEmpty()) {
                ...
                System.out.println("BOTH Portfolio and Positions are empty or null ==>Positions size:  " + wrapper.getPositions().size());
                System.out.println("BOTH Portfolio and Positions are empty or null ==>Portfolio size:  " + wrapper.getHashMap_UpdatePortfolio().size());
                System.out.println("BOTH Portfolio and Positions are empty_or_zero-size.");
                ...
                for (int cr = 0; cr < activeContracts.size(); cr++) {
                    ...
                    String commandExecuteOrNo = newStrategy.executionDeterminer(currentSymbolFUT, wrapper.getBarsHistDataArrayList());
                    System.out.println("--------------------------------------------------------------------------");
                    System.out.println("Initial_start_|__Pristine_start_|__Returned_value_commandEXECUTE-or-NO__ :" + commandExecuteOrNo);
                    System.out.println("--------------------------------------------------------------------------");

                    if ("EXECUTE".equals(commandExecuteOrNo)) {
                        String signalDecisionBS = newStrategy.getSignalDecision();
                        System.out.println("--------------------------------------------------------------------------");
                        System.out.println("Initial start | Pristine start | Returned_value_signalDecision--BUY/SELL=    " + newStrategy.getSignalDecision());
                        System.out.println("--------------------------------------------------------------------------");
                        System.out.println();
                        TimeUnit.SECONDS.sleep(5);
                        if ("SELL".equals(signalDecisionBS)) {
                            System.out.println("Order Placed --SELL  1");
                            OOP.placeOrder(currentContract, OrderTypes.MarketOrder("SELL", 1));
                        }
                        if ("BUY".equals(signalDecisionBS)) {
                            System.out.println("Order Placed --BUY  1");
...
//------- This portion of code opens positions if zero positions exist.
            ...
            toOpen = positions.zeroPositionDiscloser(wrapper.getPositions());
            System.out.println("Number of postions to open= " + toOpen.size() + " Positions needed to open: " + toOpen);

            if (toOpen.size() > activeContracts.size()) {
                System.out.println();
                System.out.println("MAJOR ERROR EXISTS");
                System.out.println("MAJOR ERROR EXISTS");
                System.out.println("MAJOR ERROR EXISTS");
                System.out.println();
            }
            ...
            for (int cr = 0; cr < toOpen.size(); cr++) {
                ...
                if (wrapper.getBarsHistDataArrayList().size() != 0) {
                    ...
                    while (newStrategy.executionDeterminer(strContract, incomingBarInput) == null) {
                        newStrategy.executionDeterminer(strContract, incomingBarInput);
                        TimeUnit.SECONDS.sleep(3);
                    }
                    String determinedDecisionExecuteOrNO = newStrategy.executionDeterminer(strContract, incomingBarInput);
                    System.out.println("Above/BelowCapacity - Zero positions checker/OPENER  | Return value determinedDecisionExecuteOrNO=    " + determinedDecisionExecuteOrNO);
                    if ("EXECUTE".equals(determinedDecisionExecuteOrNO)) {
                        TimeUnit.SECONDS.sleep(3);
                        String signalDecisionBScapacity = newStrategy.getSignalDecision();
                        System.out.println("Above/BelowCapacity - Zero positions checker/OPENER  | Return value signalDecisionBScapacity     =    " + signalDecisionBScapacity);

                        if ("SELL".equals(signalDecisionBScapacity)) {
                            System.out.println("Order Placed --SELL  1");
                            OOP.placeOrder(currentContract, OrderTypes.MarketOrder("SELL", 1));
                        }
                        if ("BUY".equals(signalDecisionBScapacity)) {
                            System.out.println("Order Placed --BUY  1");
                            OOP.placeOrder(currentContract, OrderTypes.MarketOrder("BUY", 1));
                        }
...
/*Here at this point, we assume full capacity meaning 5 contracts are open--so we assess if unRealizedPNL meets our criteria for closing position.*/
            System.out.println("------------------------------------------------------------------------------------");
            System.out.println("'Full Capacity' if applicable--|-Portfolio Assessment-|--unrealizedPNL actualization");
            System.out.println("'Full Capacity' if applicable--|-Portfolio Assessment-|--unrealizedPNL actualization");
            System.out.println("------------------------------------------------------------------------------------");
            System.out.println();
            System.out.println();
            ...
            for (Map.Entry<String, Double> currentEntry : entrySet) {
                ...
                if (assess_Position > 0.0) {
                    System.out.println("Order Placed --SELL " + integerAssess);

                    OOP.placeOrder(currentContract, OrderTypes.MarketOrder("SELL", integerAssess));
                }
                if (assess_Position < 0.0) {
                    System.out.println("Order Placed --BUY " + integerAssess);
...
/////////////code to Enforce MARGIN quantity  - begin
            ...
            System.out.println("Here's to Enforce Qty " + EnforceToClose.size());
            ...
            for (Map.Entry<String, Double> currentEntry : entrySetToEnforce) {
                ...
                if (current_Position > 0.0) {
                    System.out.println("Order Placed --BUY  " + integerValue);
                    OOP.placeOrder(currentContract, OrderTypes.MarketOrder("BUY", integerValue));
                }
                if (current_Position < 0.0) {
                    System.out.println("Order Placed --SELL  " + integerValue);
